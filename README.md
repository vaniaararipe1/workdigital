...
scripts/
  gerar_proposta.py         Script gerador (CLI)
...
propostas/                  Onde os arquivos gerados são salvos (.pptx e .pdf)
...
### Opção 1 — Pelo chat do Claude Code (recomendado)
...
- Python 3 + [`python-pptx`](https://pypi.org/project/python-pptx/)
- `soffice` (LibreOffice) com o componente **`libreoffice-impress`**
  instalado — é ele quem faz a conversão PPTX → PDF.
